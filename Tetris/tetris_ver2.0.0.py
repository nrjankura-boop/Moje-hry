...
class Queue():
    ...
    def push(self, hodnota):
        self._obsah.append(hodnota)
    
    def pop(self):
        return self._obsah.pop(0)
    
    def pocet_v_rade(self):
        return len(self._obsah)
...
class Tetris:
    ...
    def init_entity(self):
        ...
        while self.rad.pocet_v_rade() < 2:
            self.rad.push(random.choice(self.utvar))
        
        if self.prva_inicializacia: 
            self.entita = self.rad.pop()()
            self.nasledovna_entita = self.rad.pop()()
            suradnice, self.rotacia = self.entita.suradnice()
    ...
    def pohyb(self):
        if self.entita is None: self.init_entity()       
        self.vykresli()
        self.nadol()
    ...
    def vypis_score(self):
        canvas.create_text(self.x0 + (self.pocet_s * self.d) // 2, 50, text=str(self.score), font='Arial 40')
